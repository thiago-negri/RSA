...
public class MillerRabin implements PrimalityChecker {
	...
	/**
	 * Evaluates if the input is a probable prime number. Although a true output does not guarantee that input is prime, the accuracy
	 * parameter might give you some certainty. 
	 * 
	 * @param n A number N which will be tested as a probable prime
	 * @param accuracy Number of times that Miller Rabin evaluation will be performed to increase the accuracy of the primality 
	 * @return false if input number is definitely a composite (not prime); true if it is a probable prime (according to accuracy evaluation)
	 */
	...
	private boolean testMillerRabin(final BigInteger n) {
		...
		for (int i=0; i < s-1; i++) {
			...
			if (x.equals(BigInteger.ONE)) return false; //1 is NOT congruent to -1 (mod n) - This is a composite number
			if (x.equals(nMinusOne)) return true; // (n-1) is congruent to -1 (mod n) - We have a prime chance
		}
